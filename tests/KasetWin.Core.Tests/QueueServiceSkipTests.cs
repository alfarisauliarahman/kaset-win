...
public class QueueServiceSkipTests
{
    ...
    [Fact]
    public void TrySetCurrentByVideoId_AlignsIndexToQueuedTrack()
    {
        var queue = QueueOf(count: 3, startIndex: 0, RepeatMode.Off);

        bool moved = queue.TrySetCurrentByVideoId("v2");

        Assert.True(moved);
        Assert.Equal(2, queue.CurrentIndex);
        Assert.Equal("v2", queue.CurrentTrack?.VideoId);
    }

    [Fact]
    public void TrySetCurrentByVideoId_LeavesIndexWhenTrackIsNotQueued()
    {
        var queue = QueueOf(count: 3, startIndex: 1, RepeatMode.Off);

        bool moved = queue.TrySetCurrentByVideoId("missing");

        Assert.False(moved);
        Assert.Equal(1, queue.CurrentIndex);
        Assert.Equal("v1", queue.CurrentTrack?.VideoId);
    }
}
