...
public sealed partial class ArtistDetailViewModel : ViewModelBase
{
    ...
    private Task ShuffleTopSongsAsync()
    {
        ...
        KasetWin.Core.Diagnostics.KasetTrace.Log("Play:ArtistDetail.Shuffle", $"topSongs={TopSongs.Count}");
        if (TopSongs.Count == 0)
        {
    ...
    private Task StartRadioAsync()
    {
        ...
        KasetWin.Core.Diagnostics.KasetTrace.Log(
            "Play:ArtistDetail.Radio", $"playlist={_radioPlaylistId is not null} video={_radioVideoId is not null}");
    ...
    private Task PlayFromTopSongsAsync(int startIndex)
    {
        ...
        KasetWin.Core.Diagnostics.KasetTrace.Log("Play:ArtistDetail.PlayTopSongs", $"topSongs={TopSongs.Count} start={startIndex}");
        if (TopSongs.Count == 0)
        {
            return Task.CompletedTask;
        }
